...
void DCCEXParser::parse(const int comId, const char *com, const int connId)
{
  ...
  case 'T': // <T ID THROW>
    ...
    case 2:
      t = Turnout::get(comId, connId, p[0]);
      if (t != NULL)
        t->activate(comId, connId, p[1], (DCCMain *)mainTrack);
      else
        CommManager::printf(comId, connId, F("<X>"));
      break;

    // argument is string with id number of turnout followed by an address and
    // subAddress
    case 3:
      Turnout::create(comId, connId, p[0], p[1], p[2], 1);
      break;

    case 1: // argument is a string with id number only
      Turnout::remove(comId, connId, p[0]);
      break;

    case 0:                                  // no arguments
      Turnout::show(comId, connId, 1); // verbose show
      break;
    }
  ...
  case 'Z': // <Z ID ACTIVATE>
    ...
    // invert flag
    case 3:
      Output::create(comId, connId, p[0], p[1], p[2], 1);
      break;

    case 1: // argument is a string with id number only
      Output::remove(comId, connId, p[0]);
      break;

    case 0:                                 // no arguments
      Output::show(comId, connId, 1); // verbose show
      break;
    }
  ...
  case 'S':
    ...
    case 3:
      Sensor::create(comId, connId, p[0], p[1], p[2], 1);
      break;

    case 1: // argument is a string with id number only
      Sensor::remove(comId, connId, p[0]);
      break;

    case 0: // no arguments
      Sensor::show(comId, connId);
      break;
  ...
  case '1': // <1>
    ...
    CommManager::printf(comId, connId, F("<p1>"));
    break;
  ...
  case '0': // <0>
    ...
    CommManager::printf(comId, connId, F("<p0>"));
    break;
  ...
  case 'c': // <c>
    ...
    CommManager::printf(comId, connId, F("<a %d>"), currRead);
    break;

    /***** READ STATUS OF DCC++ BASE STATION  ****/

  case 's': // <s>
    CommManager::printf(comId, connId, F("<p%d MAIN>"), mainTrack->hdw.getStatus());
    CommManager::printf(comId, connId, F("<p%d PROG>"), progTrack->hdw.getStatus());
    for (int i = 1; i < mainTrack->numDevices; i++)
    {
      if (mainTrack->speedTable[i].speed == 0)
        continue;
      CommManager::printf(comId, connId, "<T%d %d %d>", i, mainTrack->speedTable[i].speed,
                          mainTrack->speedTable[i].forward);
    }
    CommManager::printf(comId, connId,
                        F("<iDCC++ BASE STATION FOR ARDUINO %s / %s: V-%s / %s %s>"),
                        "Command Station", BOARD_NAME, VERSION, __DATE__, __TIME__);
    CommManager::showInitInfo();
    Turnout::show(comId, connId);
    Output::show(comId, connId);
  ...
  case 'E': // <E>
    EEStore::store();
    CommManager::printf(comId, connId, F("<e %d %d %d>"), EEStore::eeStore->data.nTurnouts,
  ...
  case 'e': // <e>
    ...
    EEStore::clear();
    CommManager::printf(comId, connId, F("<O>"));
    break;

    /***** PRINT CARRIAGE RETURN IN SERIAL MONITOR WINDOW  ****/

  case ' ': // < >
    CommManager::printf(comId, connId, F("\n"));
    break;
  }
}
...
void DCCEXParser::cvResponse(serviceModeResponse response)
{
  ...
  case READCV:
  case WRITECV:
    CommManager::printf(response.comId, response.connId, F("<r%d|%d|%d %d>"), response.callback,
...
{
  CommManager::printf(response.comId, response.connId, F("<k %d %x>"), response.transactionID, response.data);
}